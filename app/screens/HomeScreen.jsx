  ...
  <Animated.View style={[tw`mt-4 overflow-hidden`, { height: contentHeight }]}>
    {isExpanded && (
      <View style={tw`flex-row flex-wrap justify-start  p-4 rounded-lg`}>
        {announcementItems.map((item, index) => (
          <View key={index} style={tw`items-center mr-4 mb-4`}>
            <View style={tw`w-14 h-14 bg-white rounded-full ml-6 items-center justify-center`}>
              <Text style={tw`text-black text-lg font-bold`}>{item.number}</Text>
            </View>
            <Text style={tw`mt-1 ml-6 text-white text-sm font-semibold`}>{item.label}</Text>
          </View>
        ))}
      </View>
    )}
